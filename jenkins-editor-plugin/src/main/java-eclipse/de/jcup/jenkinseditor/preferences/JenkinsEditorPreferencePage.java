...
import de.jcup.jenkinseditor.preferences.JenkinsOpenLinkInBrowserListener.URIFetcher;
...
public class JenkinsEditorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
    ...
    private void createJenkinsWaitForLogsInSeconds(Composite parent) {
        ...
        waitForLogsInSeconds.getLabelControl(parent).setToolTipText("Setup this value to handle the amount of seconds, editor will wait until fetching new console output.\n"
    ...
    private StringBuilder createStringBuilderWithConfiguredJenkinsBaseURL() {
        StringBuilder sb = new StringBuilder();
        String baseURL = null;
        String jenkinsURLFromUser = jenkinsUrl.getStringValue();
        if (StringUtils.isEmpty(jenkinsURLFromUser)) {
            baseURL = jenkinsDefaultURLProvider.getDefaultJenkinsURL();

        } else {
            baseURL = jenkinsURLFromUser;
        }
        sb.append(baseURL);
        return sb;
    }
