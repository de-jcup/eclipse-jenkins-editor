...
public class JenkinsEditorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	protected void createBracketsFieldEditors(Composite appearanceComposite) {
		...
		onlyStrictCodeCompletion = new BooleanFieldEditor(P_EDITOR_SHOW_ALSO_NON_STRICT_CODE_PROPOSALS.getId(),
				"Show also NON strict code proposals", getFieldEditorParent());
	...
	protected void createJenkinsCLIFieldEditors() {
		...
		certificateCheckDisabled = new BooleanFieldEditor(P_CERTIFICATE_CHECK_DISABLED.getId(),
				"Disable certificate check", jenkinsCLIComposite);
		addField(certificateCheckDisabled);

		BooleanFieldEditor useEclipseProxySettingsEnabled = new BooleanFieldEditor(
				P_USE_ECLIPSE_PROXY_SETTINGS_ENABLED.getId(), "Use eclipse proxy settings", jenkinsCLIComposite);
        ...
        /* @formatter:on */
		ComboFieldEditor comboFieldEditor = new ComboFieldEditor(name, labelText, entryNamesAndValues,
				jenkinsCLIComposite);
		addField(comboFieldEditor);
		...
		connectionTestButton.addSelectionListener(
				new JenkinsTestCommandExecutionListener(this, new JenkinsLinterCLICommand(), null));
		...
		if (EclipseDevelopmentSettings.DEBUG_ADD_SPECIAL_MENUS) {
			...
			JenkinsTestCommandExecutionListener listener = new JenkinsTestCommandExecutionListener(this,
					new JenkinsHelpCommand(), new HelpCommandResultHandler());
